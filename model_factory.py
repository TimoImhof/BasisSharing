...
def do_update_model(config, model, dataset, tokenizer, data_collator):
    if os.path.exists(config.updated_model_path):
        ...
        if config.model_type == "gpt2":
            model = ShareGPT2LMHeadModel.from_pretrained(
                config.updated_model_path, device_map="auto"
            )
            ...
            model = ShareOPTForCausalLM.from_pretrained(
                config.updated_model_path, device_map="auto"
            )
        ...
        std_model = AutoModelForCausalLM.from_pretrained(
            config.model_name, device_map="cpu"
        )
        std_model.config.use_cache = False
        model = load_checkpoint_and_dispatch(
            model, config.untrained_model_path, device_map="auto"
        )
        ...
        for name in names:
            ...
            model = update_model(
                ...
                step=ShareConfig.weight_info[short_model_name][
...
def create_model(config):
    if os.path.exists(config.untrained_model_path):
        ...
        for name in names:
            ...
            nx, nf = ShareConfig.weight_info[short_model_name][
                getattr(config, name + "_name")
            ]
            ...
            group = [
                [gs * i + j for j in range(config.group_size)] for i in range(num_group)
            ]
            if rest != 0:
                group += [[num_group * config.group_size + i for i in range(rest)]]
            setattr(model_config, name + "_groups", group)
            num_basis = compute_num_basis(
                nx, nf, config.group_size, config.compression_ratio
            )
        ...
        for name in names:
            ...
            setattr(
                model_config,
                name + "_groups",
                [[i] for i in range(model_config.num_hidden_layers)],
            )
            nx, nf = ShareConfig.weight_info[short_model_name][
                getattr(config, name + "_name")
            ]
        ...
        if not hasattr(config, "tfs"):
            matched_state_dict, _ = match_state_dict(
                model.state_dict(), std_model.state_dict()
            )
            ...
            if config.save_untrained_model:
                model.save_pretrained(
                    config.untrained_model_path, safe_serialization=False
                )
