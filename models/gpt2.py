...
class ShareGPT2SdpaAttention(GPT2SdpaAttention):
    def __init__(
        self, config, share_attn, share_o, is_cross_attention=False, layer_idx=None
    ):
        ...
        if share_attn:
            self.c_attn = Coefficient(
                3 * self.embed_dim, config.num_basis_attn, bias=True
            )
        if share_o:
...
class ShareGPT2MLP(GPT2MLP):
    def __init__(self, intermediate_size, config, share_up, share_down, layer_idx):
        ...
        if share_down:
            self.c_proj = Coefficient(
                config.hidden_size, config.num_basis_down, bias=True
            )

    def forward(
        self, hidden_states: Optional[Tuple[torch.FloatTensor]], **kwargs
    ) -> torch.FloatTensor:
        if self.share_up:
...
class ShareGPT2Block(GPT2Block):
    def __init__(self, config, layer_idx=None):
        ...
        self.attn = ShareGPT2SdpaAttention(
            config, share_attn, share_o, layer_idx=layer_idx
        )
...
class ShareGPT2Model(GPT2Model):
    def __init__(self, config):
        ...
        if hasattr(config, "num_basis_attn"):
            self.attn_basis = build_basis_collection(
                config.attn_groups, config.num_basis_attn, config.hidden_size
            )
        else:
        ...
        if hasattr(config, "num_basis_down"):
            self.down_basis = build_basis_collection(
                config.down_groups, config.num_basis_down, config.hidden_size * 4
            )
        else:
            self.down_basis = None

        self.h = torch.nn.ModuleList(
            [ShareGPT2Block(config, i) for i in range(config.num_hidden_layers)]
        )
