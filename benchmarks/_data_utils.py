...
def prepare_data(dataset_name, tokenizer, context_length, dataset_cache_dir=None):
    if dataset_name == "wikitext":
        train_dataset, val_dataset, tokenized_test, data_collator = (
            prep_wikitext_2_raw_v1(context_length, tokenizer, dataset_cache_dir)
        )
    elif dataset_name == "ptb":
        train_dataset, val_dataset, tokenized_test, data_collator = prep_ptb(
            context_length, tokenizer, dataset_cache_dir
        )
    elif dataset_name == "c4":
        train_dataset, val_dataset, tokenized_test, data_collator = prep_c4(
            context_length, tokenizer, dataset_cache_dir
        )
    elif dataset_name == "alpaca":
        train_dataset, val_dataset, tokenized_test, data_collator = prep_alpaca(
            context_length, tokenizer, dataset_cache_dir
        )
    else:
...
def prep_wikitext_2_raw_v1(context_length, tokenizer, dataset_cache_dir=None):
    ...
    tokenized_train = train_raw_dataset.map(
        func, num_proc=4, batched=True, remove_columns="text"
    )
    tokenized_val = val_raw_dataset.map(
        func, num_proc=4, batched=True, remove_columns="text"
    )
    ...
    train_dataset = tokenized_train.map(func, num_proc=4, batch_size=256, batched=True)
    val_dataset = tokenized_val.map(func, num_proc=4, batch_size=256, batched=True)
...
def prep_ptb(context_length, tokenizer, dataset_cache_dir=None):
    ...
    tokenized_train_data = train_raw_dataset.map(
        func, num_proc=4, batched=True, remove_columns="sentence"
    )
    tokenized_val_data = val_raw_dataset.map(
        func, num_proc=4, batched=True, remove_columns="sentence"
    )
    ...
    func = partial(group_text, context_length=context_length)
    train_dataset = tokenized_train_data.map(
        func, num_proc=4, batch_size=256, batched=True
    )
    val_dataset = tokenized_val_data.map(func, num_proc=4, batch_size=256, batched=True)
    data_collator = DataCollatorForLanguageModeling(
        tokenizer, mlm=False, return_tensors="pt"
    )
...
def prep_alpaca(context_length, tokenizer, dataset_cache_dir=None):
    ...
    train_raw_dataset = load_dataset(
        "tatsu-lab/alpaca", split="train", dataset_cache_dir=dataset_cache_dir
    )
    ...
    func = partial(group_text, context_length=context_length)
    train_dataset = tokenized_train_data.map(
        func, num_proc=1, batch_size=256, batched=True
    )
    data_collator = DataCollatorForLanguageModeling(
        tokenizer, mlm=False, return_tensors="pt"
    )
